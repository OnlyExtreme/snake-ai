...
class Board(object):
    ...
    def generate_food(self):    # Randomly Generate a new food
    ...
    def turn(self, ndir):   # Check and do a turn
        cdir = self.dir_head
        if (cdir & 1) != (ndir & 1):
            self.dir_head = ndir
    ...
    def get_sensor(self):   # Get sensing data for the input of the neural networks
        sensor = []
        ft = self.get_tag(0)
        lt = self.get_tag(1)
        rt = self.get_tag(2)
        sensor.append(float(ft == -1))
        sensor.append(float(lt == -1))
        sensor.append(float(rt == -1))
        sensor.append(float(ft != 0 and ft != -1))
        sensor.append(float(lt != 0 and lt != -1))
        sensor.append(float(rt != 0 and rt != -1))
        return sensor
...
class Game(object):
    ...
    def __init__(self, is_Human_Player):
        self.score = 0
        self.game_over = 0
